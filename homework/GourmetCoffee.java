...
public class GourmetCoffee {
	...
	public void displayNumberOfOrders(Product product) {
		...
		int res = 0;
		Iterator<Order> order = sales.iterator();
		while (order.hasNext()) {
			Iterator<OrderItem> orderitem = order.next().iterator();
			while (orderitem.hasNext()) {
				if (orderitem.next().getProduct().equals(product)) {
					res++;
				}
			}
		}
		stdOut.println("Number of orders that contains the product " + product.getCode() + ": " + res);
	}
	...
	public void displayTotalQuantityOfProducts() {
		...
		for (Product product : this.catalog) {
			...
			Iterator<Order> order = sales.iterator();
			while (order.hasNext()) {
				OrderItem it = order.next().getItem(product);
				if (it != null) {
					res += it.getQuantity();
				}
			}
			stdOut.println(product.getCode() + " " + res);
		}
	}
